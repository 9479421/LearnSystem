...
const banner = new URL('../assets/images/logo-banner.svg', import.meta.url).href
...
const loginRules = ref<FormRules>({
  ...
  password: [
    ...
    { min: 4, max: 18, trigger: 'blur', message: '密码长度为4到18位' },
  ],
})
...
function handleRegister() {
  registerFormRef.value && registerFormRef.value.validate((valid) => {
    if (valid) {
      // 这里编写业务代码
      userStore.register(registerForm.value).then((res:any)=>{
        if(res.code==200){
...
const registerCd = ref(false);
const registerText = ref('发送验证码')
function sendRegisterCode(){
  userStore.sendRegisterCode(registerForm.value.phone);
  registerCd.value= true;
  //开始计时
  var seconds = 60;
  registerText.value = seconds--+'秒后再发送'
  var t = setInterval(function(){
    registerText.value = seconds--+'秒后再发送'
    if(seconds<=0){
      //恢复
      registerText.value = '发送验证码'
      registerCd.value= false;
      clearInterval(t)
    }
  },1000)
}
...
function forget(){
  ElMessageBox.alert('该功能暂未开放，请联系作者', '提示', {
    ...
    <div id="login-box">
      ...
      >
        ...
        <div class="flex-bar">
          ...
          <el-link type="primary" :underline="false" @click="forget()">忘记密码了?</el-link>
        </div>
        <el-button
          :loading="loading"
          type="primary"
        ...
        <div>
          ...
          <el-form-item prop="captcha">
            ...
            >
              ...
              <template #append>
                <el-button @click="sendRegisterCode()" type="primary" :disabled="registerForm.phone.length!=11 || registerCd">{{registerText}}</el-button>
              </template>
            </el-input>
          </el-form-item>
          <el-form-item prop="password">
